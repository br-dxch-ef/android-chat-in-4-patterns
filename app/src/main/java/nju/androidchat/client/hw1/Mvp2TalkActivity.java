        ...
        runOnUiThread(() -> {
                    ...
                    for (ClientMessage message : messages) {
                        ...
                        if(text.matches(img_pattern)){
                            String img_url = text.substring(text.indexOf('{') + 1, text.indexOf('}'));
                            System.out.println("img_url: " + img_url);
                            // 如果是自己发的，增加ItemImgSend
                            if (message.getSenderUsername().equals(this.presenter.getUsername())) {
                                content.addView(new ItemImgSend(this, img_url, message.getMessageId(), this));
                            } else {
                                content.addView(new ItemImgReceive(this, img_url, message.getMessageId()));
                            }
                        }
                        else{
